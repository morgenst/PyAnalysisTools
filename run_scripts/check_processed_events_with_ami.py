...
def get_size(dataset_name):
    ...
    _logger.debug("Parsing info for {:s}".format(dataset_name))
    dataset_info = client.execute("GetDatasetInfo  -logicalDatasetName={:s}".format(dataset_name),
...
def main(argv):
    ...
    expected_yields = {get_process(dataset_name): get_size(dataset_name) for dataset_name in
    ...
    difference = {process: yields[process] - expected_yields[process] if process in expected_yields else yields[process]
                  for process in list(yields.keys())}
    difference = dict([kv for kv in iter(difference.items()) if kv[1] != 0.])
