...
from PyAnalysisTools.AnalysisTools.FitHelpers import get_hist_from_canvas, plot_all_components, format_and_draw_frame, \
    add_parameters_to_canvas, create_roodata
...
class PDF2Gauss(PDF):
    ...
    def build(self):
        ...
        if "Ds" in self.mode:
            w.factory(
                "EXPR::background('exp(decayrate*triplet_refitted_m+decayrate2*triplet_refitted_m*triplet_refitted_m)',"
                "decayrate[-1.5e-3, -0.01, -1e-4], decayrate2[3e-7, 1e-10, 3e-6], triplet_refitted_m)")
            ...
            if "Data" in self.mode:
                w.factory(
                    "SUM::model(nD[500,0,20000]*gauss1, nDs[2000,0,20000]*gauss2, nBkg[10000,0,10000000]*background)")
                w.factory("EDIT::model(model, nD=expr('alpha*nDs', alpha[0.3,0.1,0.5], nDs))")
        if "Phi" in self.mode:
            w.factory(
                "EXPR::background('exp(decayrate*triplet_muon_mass+decayrate2*triplet_muon_mass*triplet_muon_mass)',"
                "decayrate[1e-7, 1.], decayrate2[1e-7, -1e-6, 1e-6], triplet_muon_mass)")
...
class PDFBFraction(PDF):
    ...
    def build(self):
        ...
        # binlist = array.array('d', [-4+j*0.25 for j in xrange(24)]+[2. + i*0.5 for i in xrange(4)]+
        # [4. + i*1 for i in xrange(11)])
...
class Fitter(object):
    def __init__(self, **kwargs):
        ...
        self.logy = False
        self.pdf_config = PDFConfig(fit_config_file=kwargs["fit_config_file"]) if "fit_config_file" in kwargs \
            else kwargs["fit_config_config"]
        self.mode = kwargs["mode"]
    ...
    def fit(self, return_fit=False, extra_selection=[]):
        ...
        self.data = create_roodata(self.file_handles, self.tree_name, self.var, self.quantity, self.blind,
                                   copy.deepcopy(self.selection), extra_selection, self.weight)
        self.build_model()
        if self.blind:
